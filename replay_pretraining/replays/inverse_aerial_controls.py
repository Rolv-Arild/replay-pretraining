...
def aerial_inputs(omega_start, omega_end, theta_start, dt):
    ...
    tau = np.dot(tst, tau)  # net torque in local coordinates
    omega_local = np.dot(tst, omega_start)  # beginning-step angular velocity in local coordinates
...
def pyr_from_dataframe(deltas, player_df):
    is_repeated = (player_df[["pos_x", "pos_y", "pos_z",
                              ...
                              "quat_w", "quat_x", "quat_y", "quat_z"]].diff() == 0).all(axis=1)
    ang_vels = player_df[['ang_vel_x', 'ang_vel_y', 'ang_vel_z']]
    quats = player_df[['quat_w', 'quat_x', 'quat_y', 'quat_z']]
    pyrs = np.zeros((len(deltas), 3))
    i = 0
    while i < len(deltas) - 1:
        omega_start = ang_vels.iloc[i].values
        theta_start = quat_to_rot_mtx(quats.iloc[i].values)
        j = i
        delta = 0
        while True:
            j += 1
            delta += deltas.iloc[j]
            if not is_repeated.iloc[j] or j >= len(deltas) - 1:
                break
        if delta <= 0:
            pyrs[i] = np.nan
        else:
            omega_end = ang_vels.iloc[j].values
